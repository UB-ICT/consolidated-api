...
class MenuController extends Controller
{
     ...
     */
    public function userMenusByUser(string $user): JsonResponse
    {
        $targetUserQuery = User::query();

        // Avoid Postgres UUID cast errors when identifier is not a UUID.
        if (Str::isUuid($user)) {
            $targetUserQuery->where('id', $user);
        }

        $targetUser = $targetUserQuery
            ->orWhere('guid', $user)
            ->orWhere('email', $user)
            ->first();
